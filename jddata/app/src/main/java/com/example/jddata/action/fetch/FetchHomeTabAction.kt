...
class FetchHomeTabAction(env: Env) : BaseAction(env, ActionType.FETCH_HOME_TAB) {

    init {
        appendCommand(Command().commandCode(ServiceCommand.FETCH_TAB))
    }

    override fun initLogFile() {
        logFile = BaseLogFile("获取_" + GlobalInfo.HOME_TAB)
    }
            ...
            for (list in lists) {
                ...
                do {
                    ...
                    if (AccessibilityUtils.isNodesAvalibale(items)) {
                        val scrolls = AccessibilityUtils.findChildByClassname(mService!!.rootInActiveWindow, "android.widget.HorizontalScrollView")
                        for (scroll in scrolls) {
                            val names = AccessibilityUtils.getAllText(scroll);
                            for (i in names.indices step 2) {
                                ...
                                row.tab = tabName
                                row.biId = GlobalInfo.HOME_TAB
                                row.itemIndex = "${itemCount}"
                                LogUtil.dataCache(row)

                                logFile?.writeToFileAppend("收集${itemCount}标签：", tabName)
                            }
                            return true
                        }
                    }
